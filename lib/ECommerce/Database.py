...
def create_sample_data():
    ...
    ⚠️ WARNING: This function creates users with default passwords!
    
    SECURITY NOTICE:
    - Only use this function in DEVELOPMENT environments
    - NEVER run this in PRODUCTION
    - Delete all sample users before deploying to production
    - Change all default passwords immediately after setup
    
    To disable: Comment out the call to this function in initialize_database()
    """
    from lib.ECommerce.Models.User import User
    from lib.ECommerce.Models.Customer import Customer
    from lib.ECommerce.Models.Product import Product
    ...
    # Security check: Don't create sample data in production
    if not settings.DEBUG:
        print("⚠️  Skipping sample data creation (DEBUG=False)")
        print("   Sample data should only be created in development environments")
        return

    # Create default users (DEVELOPMENT ONLY)
    # ⚠️ WARNING: These passwords are weak and for development only!
    users_data = [
