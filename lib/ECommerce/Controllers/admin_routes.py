...
def order_update_status(request, order_id):
    """Update order status."""
    from lib.ECommerce.Models.Order import OrderTimeline
    ...
    if new_status and new_status != order.status:
        ...
        OrderTimeline.add_event(order, new_status, description, request.user)
...
def customers(request):
    ...
    # Annotate customers with order count and total spent
    customers_list = customers_list.annotate(
        order_count=Count('orders'),
        total_spent=Sum('orders__total', filter=~models.Q(orders__status='cancelled'))
    )
    ...
    if sort == 'newest':
        ...
        customers_list = customers_list.order_by('-order_count')
    elif sort == 'spent':
        customers_list = customers_list.order_by('-total_spent')
    elif sort == 'name':
        customers_list = customers_list.order_by('first_name', 'last_name')
    ...
    active_customers = Customer.objects.filter(
        orders__created_at__gte=today - timedelta(days=30)
    ).distinct().count()
...
def reports(request):
    ...
    export_report = request.GET.get('export_report', 'all')
    ...
    if export_type == 'csv':
        ...
        response['Content-Disposition'] = f'attachment; filename="report_{period}_{start_date}_to_{end_date}.csv"'
        
        writer = csv.writer(response)
        
        if export_report in ['all', 'summary']:
            writer.writerow(['=== REPORT SUMMARY ==='])
            writer.writerow(['Period', f'{start_date} to {end_date}'])
            writer.writerow(['Total Revenue', f'${total_revenue:.2f}'])
            writer.writerow(['Total Orders', total_orders])
            writer.writerow(['Average Order Value', f'${avg_order_value:.2f}'])
            writer.writerow(['Products Sold', products_sold])
            writer.writerow(['Unique Products', unique_products])
            writer.writerow(['New Customers', new_customers])
            writer.writerow([])
        
        if export_report in ['all', 'top_products']:
            writer.writerow(['=== TOP SELLING PRODUCTS ==='])
            writer.writerow(['Rank', 'Product', 'Quantity Sold', 'Revenue'])
            for i, p in enumerate(top_products, 1):
                writer.writerow([i, p['name'], p['quantity_sold'], f"${p['revenue']:.2f}"])
            writer.writerow([])
        
        if export_report in ['all', 'top_customers']:
            writer.writerow(['=== TOP CUSTOMERS ==='])
            writer.writerow(['Rank', 'Customer', 'Orders', 'Total Spent'])
            for i, c in enumerate(top_customers_list, 1):
                writer.writerow([i, f"{c['first_name']} {c['last_name']}", c['order_count'], f"${c['total_spent']:.2f}"])
            writer.writerow([])
        
        if export_report in ['all', 'revenue']:
            writer.writerow(['=== DAILY REVENUE ==='])
            writer.writerow(['Date', 'Revenue'])
            for label, data in zip(revenue_labels, revenue_data):
                writer.writerow([label, f'${data:.2f}'])
            writer.writerow([])
        
        if export_report in ['all', 'categories']:
        ...
        # For PDF, we'll generate a simple HTML response that can be printed to PDF
        from django.http import HttpResponse
        ...
        <html>
        <head>
            <title>Report - {start_date} to {end_date}</title>
            <style>
        ...
        <body>
            ...
            <div class="summary">
                <div class="summary-item">
                    ...
                    <div class="summary-value">${total_revenue:,.2f}</div>
                </div>
                <div class="summary-item">
                ...
                <div class="summary-item">
                    ...
                    <div class="summary-value">${avg_order_value:,.2f}</div>
                </div>
                <div class="summary-item">
                    <div class="summary-label">Products Sold</div>
                    <div class="summary-value">{products_sold}</div>
                </div>
            </div>
            ...
            <table>
                ...
                {"".join(f"<tr><td>{i}</td><td>{p['name']}</td><td>{p['quantity_sold']}</td><td>${p['revenue']:,.2f}</td></tr>" for i, p in enumerate(top_products, 1))}
            </table>
            
            <h2>Top Customers</h2>
            <table>
                <tr><th>Rank</th><th>Customer</th><th>Orders</th><th>Total Spent</th></tr>
                {"".join(f"<tr><td>{i}</td><td>{c['first_name']} {c['last_name']}</td><td>{c['order_count']}</td><td>${c['total_spent']:,.2f}</td></tr>" for i, c in enumerate(top_customers_list, 1))}
            </table>
